...
module p2p_250mhz #(
  ...
  // Graph aggregator idle-flush window (~1 ms @ 250 MHz). Overridden to a
  // small value by the simulation testbench.
  parameter int AGGR_FLUSH_TIMEOUT = 250000
) (
...
`endif
  ...
  generate for (genvar i = 0; i < NUM_INTF; i++) begin
    ...
    if (NUM_QDMA > 1) begin
      ...
      // Graph plugin: network-ingress extract + aggregate, deliver to host.
      // Replaces the pass-through RX pipeline of the stock p2p plugin.
      wire [15:0] aggr_frame_size;

      assign axis_qdma_c2h_tuser[0+:16]  = aggr_frame_size;
      assign axis_qdma_c2h_tuser[16+:16] = 16'h1 << (6 + i);  // src: CMAC port i
      assign axis_qdma_c2h_tuser[32+:16] = 16'h0;             // dst driven above
